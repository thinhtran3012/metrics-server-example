...
#include "../collector/ngx_metric_collect_cpu_stats.h"
#include "../collector/ngx_metric_collect_disk_stats.h"
#include "../collector/ngx_metric_collect_disk_utilization_info.h"
#include "../collector/ngx_metric_collect_loadavg_stats.h"
#include "../collector/ngx_metric_collect_mem_info.h"
#include "../collector/ngx_metric_collect_net_stats.h"
#include "../collector/ngx_metric_collect_netdev_stats.h"
#include "../collector/ngx_metric_collect_rtmp_stats.h"
#include "../collector/ngx_metric_collect_uname_info.h"
#include "../collector/ngx_metric_collect_vm_stats.h"
#include "../collector/ngx_metric_collect_live_session_stats.h"
...
// temp remove stats for all (*)
    ...
    size_t metrics_max_size = vtscf->metrics_size;
    ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "metrics_max_size: %uz!!!\n", metrics_max_size);

    if(vtscf->disk_utilization)
    {
        /* Get disk utilization info*/
        u_char* tmp = ngx_pcalloc(r->pool, metrics_max_size);
        ngx_http_get_disk_utilization_info(r, tmp);
        metrics_size += ngx_strlen(tmp);
        if(metrics_size > metrics_max_size)
        {
            ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "Not enough memory.");
            return buf;
        }
        else 
        {
            buf = ngx_sprintf(buf, tmp);
        }
    ...
    if(vtscf->load_avg)
    {
        /* Get loadavg stats */
        u_char* tmp = ngx_pcalloc(r->pool, metrics_max_size);
        ngx_http_get_loadavg_stats(r, tmp);
        metrics_size += ngx_strlen(tmp);
        if(metrics_size > metrics_max_size)
        {
            ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "Not enough memory.");
    ...
    if(vtscf->netdev_stats)
    {
        /* Get netdev stats */
        u_char* tmp = ngx_pcalloc(r->pool, metrics_max_size);
        ngx_http_get_netdev_stats(r, tmp);
        metrics_size += ngx_strlen(tmp);
        if(metrics_size > metrics_max_size)
        {
            ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "Not enough memory.");
    ...
    if(vtscf->live_session_stats)
    {
        u_char* tmp = ngx_pcalloc(r->pool, metrics_max_size);
        ngx_http_get_live_session_stats(r, tmp);
        metrics_size += ngx_strlen(tmp);
        if(metrics_size > metrics_max_size)
        {
            ngx_log_error(NGX_LOG_INFO, r->connection->log, 0, "Not enough memory.");
